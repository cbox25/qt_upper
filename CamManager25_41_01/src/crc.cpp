...
/* Used to write CRC check algorithms to determine whether a data packet is complete */

int check_crc8(uint8_t *buffer, uint32_t len, uint8_t crc)
{
	return crc == calc_crc8(buffer, len);
}

int check_crc16(uint8_t *buffer, uint32_t len, uint16_t crc)
{
	return crc == calc_crc16(buffer, len);
}

int check_crc32(uint8_t *buffer, uint32_t len, uint32_t crc)
{
	return crc == calc_crc32(buffer, len);
}
...
/*
 * uint8_t crc_8( const uint8_t *buf, size_t bytes );
 *
...
/*
 * uint16_t crc_16( const uint8_t *buf, size_t bytes );
 *
...
uint16_t calc_crc16(const uint8_t *buf, uint32_t bytes)
{
	...
	if (ptr != NULL) {
		for (a = 0; a < bytes; a++) {
			crc = (crc >> 8) ^ crc_table16[(crc ^ (uint16_t)*ptr++) & 0x00FF];
		}
	}
...
/*
 * uint32_t crc_32( const uint8_t *buf, size_t bytes );
 *
...
uint32_t calc_crc32(const uint8_t *buf, uint32_t bytes)
{
	...
	if (ptr != NULL) {
		for (a = 0; a < bytes; a++) {
			crc = (crc >> 8) ^ crc_table32[(crc ^ (uint32_t)*ptr++) & 0x000000FFul];
		}
	}
