...
UiLabelData g_labelData[2][READ_ONLY_DATA_NUM_MAX] = {
    {
        {NULL, NULL, "门铃数据",     0x44A00128, ""},
        {NULL, NULL, "nwrite数据", 0x44A0012C, ""},
        {NULL, NULL, "版本号",     0x44A00800, ""}
    },
...
UiLabelData g_labelData[READ_ONLY_DATA_NUM_MAX] = {
    ...
    {NULL, NULL, "温度", 0x44A209F4, ""},
    {NULL, NULL, "图像均值", 0x44A20A08, ""}
};
...
void RefreshTempBtnClickedEvent(void)
{
    ...
    // Record temperature
    QString logMessage = QString("刷新寄存器: 名称=%4, 地址=0x%5, 新值=%6")
    ...
    // Record k
    logMessage = QString("刷新寄存器: 名称=%4, 新值=%5")
...
QScrollArea *CreateRegCtrlWidgets(QWidget *pageRegCtrl, int tabNum)
{
    ...
    if (tabNum == TAB_PAGE_REG) {
        ...
        scroll->setStyleSheet("QWidget { background: transparent !important; }"); // 设置样式

        // Dynamically calculate the height of scrollContent
        int totalHeight = labelY + (labelH + spaceH) * regNumVisible + spaceH ;
        scroll->setMinimumHeight(totalHeight);
        scrollArea->setMinimumHeight(pageRegCtrl->height()); // 固定 QScrollArea 高度
        scrollArea->setWidget(scroll); // 重新设置 widget
