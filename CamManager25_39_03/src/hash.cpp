...
// Byte swap (big-endian to little-endian conversion)
#define BYTESWAP32(x) (((x) >> 24) | (((x) & 0xff0000) >> 8) | (((x) & 0xff00) << 8) | ((x) << 24))
...
void sha256_transform(SHA256_CTX *ctx, const uint8_t data[]) {
    ...
    for (; i < 64; ++i) {
        m[i] = m[i - 16] + m[i - 7] +
               (ROTRIGHT(m[i - 15], 7) ^ ROTRIGHT(m[i - 15], 18) ^ (m[i - 15] >> 3)) +
               (ROTRIGHT(m[i - 2], 17) ^ ROTRIGHT(m[i - 2], 19) ^ (m[i - 2] >> 10));
    }
...
void sha256_final(SHA256_CTX *ctx, uint8_t hash[]) {
    ...
    // Add message length (expressed in big-endian order)
    ...
    // Convert the state to big-endian order and copy it to the hash value
    for (i = 0; i < 4; ++i) {
